...
public class PVPManager {
   ...
   public static void setupFile(GrisesiaCombat gCombat) {
      main = gCombat;
   ...
   public static List<UUID> GetPlayers() {
      ...
      strings.addAll(getModifyPVPFile().getConfigurationSection("pvp-tags").getKeys(false));
      List<UUID> uuids = new ArrayList();

      for(int i = 0; i < strings.size(); ++i) {
         uuids.add(UUID.fromString((String)strings.get(i)));
      }
   ...
   public static boolean GetPlayer(Player player) {
      return getModifyPVPFile().getBoolean("pvp-tags." + player.getUniqueId());
   }
